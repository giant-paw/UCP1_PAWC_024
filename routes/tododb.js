...
router.post('/', (req, res) => {
    ...
    db.query('INSERT INTO animals (task) VALUES (?)', [task.trim()], (err, results) => {
        if (err) return res.status(500).send('Internal Server Error');
...
router.put('/:id', (req, res) => {
    ...
    db.query('UPDATE animals SET task = ?, completed = ? WHERE id = ?', [task, completed, req.params.id], (err, results) => {
        if (err) return res.status(500).send('Internal Server Error');
...
router.delete('/:id', (req, res) => {
    db.query('DELETE FROM animals WHERE id = ?', [req.params.id], (err, results) => {
        if (err) return res.status(500).send('Internal Server Error');
        if (results.affectedRows === 0) return res.status(404).send('Tugas tidak ditemukan');
        res.status(204).send();
    });
